...
BPF_HASH(start_time, int, int64_t);
BPF_HASH(end_time, int, int64_t);
...
if __name__ == '__main__':
    ...
    try:
        ...
        start_time          = b.get_table("start_time")
        end_time          = b.get_table("end_time")
        ...
        while True:
            try:
                dropcnt.clear()
                # time.sleep(1)
                for s, e in zip(start_time.values(), end_time.values()):
                    print(s.value, e.value, e.value - s.value)
